...
const emptyForm = { invoice_id: "", date: new Date().toISOString().split("T")[0], reason: "" };
...
const SalesReturnsPage = () => {
  ...
  const loadReturns = useCallback(async () => {
    setLoading(true);
    const response = await window.api?.returns?.salesSearch({ query, branch_id: currentBranch?.id || 1, page: 1, limit: 20 });
  ...
  const addToCart = () => {
    ...
    setCart((current) => {
      ...
      return [...current, { product_id: String(product.id), name: product.name, qty: Number(form.qty), unit_price: Number(product.sale_price || 0) }];
    });
  ...
  const createReturn = async (event) => {
    ...
    setSaving(true);
    const response = await window.api?.returns?.salesCreate({
  ...
  const columns = [
    ...
    { key: "actions", header: "", align: "end", render: (_, row) => <div style={{ display: "flex", justifyContent: "flex-end", gap: 6 }}><Button size="sm" variant="ghost" icon={Printer} onClick={() => window.print()} /><Button size="sm" variant="ghost" icon={Trash2} onClick={() => setDeleteId(row.id)} style={{ color: "#ef4444" }} /></div> },
  ];

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div><h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-main)" }}>مرتجعات البيع</h1><p style={{ color: "var(--text-muted)", marginTop: 4 }}>مرتجعات من فواتير البيع مع تحديث المخزون</p></div>
        <Button icon={FileText} onClick={() => setModalOpen(true)}>إضافة مرتجع</Button>
      </div>
      <Card>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}><Input icon={Search} placeholder="ابحث برقم المرتجع أو الفاتورة أو العميل" value={query} onChange={(event) => setQuery(event.target.value)} /><span style={{ color: "var(--text-muted)", whiteSpace: "nowrap", fontSize: 13 }}>{result.total || 0} مرتجع</span></div>
            ...
            <div style={{ border: "1px solid var(--border-color)", borderRadius: 12 }}>
              {cart.map((item, idx) => (
                <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 100px 120px 40px", alignItems: "center", gap: 10, padding: "12px 14px", borderBottom: "1px solid var(--border-color)", color: "var(--text-main)" }}>
                  <span>{item.name}</span><span>{item.qty} × {Number(item.unit_price).toFixed(2)}</span>
