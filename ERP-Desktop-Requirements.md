...
- **الدولة (Country)** — قائمة منسدلة بكل دول العالم؛ اختيار الدولة يُستخدم لتوليد/تصفية القيم الافتراضية التالية تلقائيًا (العملة، نسبة الضريبة، تنسيق التاريخ الشائع، الدليل المحاسبي المقترح) مع إمكانية التعديل اليدوي لاحقًا
- **العملة (Currency)** — قائمة بكل عملات العالم (رمز ISO + الرمز المحلي + عدد الخانات العشرية)، مع إمكانية دعم أكثر من عملة للتعامل (عملة أساسية للتقارير + عملات تعامل بسعر صرف يُحدَّث يدويًا أو تلقائيًا)
- **نوع التاريخ (Calendar Type)** — اختيار بين: ميلادي (Gregorian) / هجري (Hijri) / كلاهما معًا (عرض مزدوج)، مع تنسيق العرض (يوم/شهر/سنة، شهر/يوم/سنة، سنة-شهر-يوم) قابل للاختيار أيضًا
- **نوع الخط (Font Family)** — قائمة خطوط عربية/إنجليزية مدعومة (تحمّل محليًا داخل التطبيق وليس من الإنترنت)، والاختيار **يُطبَّق فورًا على كامل التطبيق** (كل الشاشات، الفواتير المطبوعة، التقارير المُصدَّرة) عبر متغير CSS عام (Global Font Variable) وليس تصميمًا مبعثرًا لكل صفحة
- **اللغة الافتراضية** (عربي/إنجليزي) — قابلة للتبديل الفوري بشكل مستقل عن اختيار الدولة (فرع مصري بلغة إنجليزية مثلاً)
...
- **إعدادات الضرائب** — عند اختيار الدولة، تُحمَّل تلقائيًا كل أنواع الضرائب المعتمدة لتلك الدولة (راجع القسم 19 للتفاصيل الكاملة) مع تحديد الضريبة الافتراضية، وإمكانية التفعيل/التعطيل والتعديل اليدوي لكل نسبة
- **رقم تسلسلي للفواتير والمستندات** قابل للتخصيص (Prefix/Format)
- **الدليل المحاسبي (Chart of Accounts Template)** — عند اختيار الدولة، يُعرض دليل محاسبي جاهز ومعتمد لتلك الدولة (راجع القسم 19) مع خيار "تطبيق الدليل" الذي ينشئ شجرة الحسابات كاملة تلقائيًا في قاعدة البيانات دفعة واحدة، أو البدء بدليل عام قابل للتعديل الكامل
...
| الجدول | الأعمدة الأساسية |
|---|---|
| `tax_rates` | id, country_code, tax_code, name_ar, name_en, tax_type (vat/gst/sales_tax/withholding/other), rate_percentage, applies_to, is_default, is_withholding, effective_from, effective_to, is_active |
